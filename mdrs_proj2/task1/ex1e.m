...
for i = 1:nNodes
    for j = i+1:nNodes
        ...
        averageRoundTripDelayAnycast = mean(delays(anycastFlows)) * 2 * 1000;
        
        % Imprimir resultados intermediários
        fprintf('Anycast nodes: %d  %d\nAverage round-trip delay: %.2f ms\n', anycastNodes(1), anycastNodes(2), averageRoundTripDelayAnycast);
        
        % Calcular as cargas dos links
        Loads = calculateLinkLoads(nNodes, Links, Taux, sP, ones(nFlows, 1));
        
        % Atualizar os melhores resultados se o atraso médio de ida e volta for menor
        if averageRoundTripDelayAnycast < minAverageRoundTripDelay
            minAverageRoundTripDelay = averageRoundTripDelayAnycast;
            bestAnycastNodes = anycastNodes;
            bestDelays = delays;
            bestTaux = Taux;
            bestSP = sP;
            minWorstLinkLoad = max(max(Loads(:, 3:4)));
        end
    end
end
