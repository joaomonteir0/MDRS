...
for i = 1:nNodes
    for j = i+1:nNodes
        ...
        worstRoundTripDelayAnycast = max(delays(anycastFlows)) * 2 * 1000;
        
        % Atualizar os melhores resultados se o atraso de ida e volta for menor
        if worstRoundTripDelayAnycast < minWorstRoundTripDelay
            minWorstRoundTripDelay = worstRoundTripDelayAnycast;
